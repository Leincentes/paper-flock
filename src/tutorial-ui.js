...
function queryOptions() {
  ...
  return {
    force: params.get("tutorial") === "1",
    bypass:
      params.get("tutorial") === "0" ||
      TEST_QUERY_KEYS.some((key) => params.get(key) === "1")
  };
}
...
function installReplayButton() {
  const nav = document.querySelector(".beta-footer nav");
  ...
  const feedback = nav.querySelector("#beta-feedback-button");
  nav.insertBefore(button, feedback ?? null);
}
